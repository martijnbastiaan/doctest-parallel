...
runModules modConfig nThreads implicitPrelude parseArgs evalArgs modules = do
  ...
  (input, output) <-
    makeThreadPool
      ...
      (runModule modConfig implicitPrelude parseArgs evalArgs)
...
runModule
  ...
  -> ModuleName
  -> IO ()
runModule modConfig0 implicitPrelude parseArgs evalArgs output modName = do
  threadId <- myThreadId
  let update r = writeChan output (threadId, r)

  mod_@(Module module_ setup examples0 modArgs) <- getDocTests parseArgs modName
